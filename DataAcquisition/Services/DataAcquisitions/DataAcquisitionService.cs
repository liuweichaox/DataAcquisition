        ...
        : IDataAcquisitionService
    {
        ...
        private async Task DataCollectAsync(DataAcquisitionConfig config, IPlcClient plcClient, IQueueManager queueManager)
        {
            try
            {
                ...
                {
                    var operationResult = await plcClient.ReadAsync(registerGroup.BatchReadAddress, registerGroup.BatchReadLength);
                    if (!operationResult.IsSuccess)
                    {
                        _ = messageService.SendAsync(
                            $"{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} - 读取 {registerGroup.BatchReadAddress} 失败");
                    }

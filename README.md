...
系统配置分为两部分：设备配置和 Edge Agent 应用配置。详细配置说明请参考以下文档：

### 配置文档

- 📖 [设备配置说明](docs/configuration/device-config.md) - 设备配置文件（PLC 配置）的详细说明
- ⚙️ [Edge Agent 应用配置](docs/configuration/edge-agent-config.md) - Edge Agent 的 appsettings.json 配置说明
- 📊 [配置到数据库映射](docs/configuration/database-mapping.md) - 配置文件如何映射到 InfluxDB 数据库

### 快速配置

1. **设备配置**: 在 `src/DataAcquisition.Edge.Agent/Configs/` 目录创建 `*.json` 格式的设备配置文件
2. **应用配置**: 编辑 `src/DataAcquisition.Edge.Agent/appsettings.json` 配置 InfluxDB、Parquet 等参数

> **提示**: 系统支持配置热更新，修改配置文件后无需重启服务。
